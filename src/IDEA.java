...
// The following condition is met for all values of x: mul(x, mulInv(x)) == 1
    ...
    public void invertSubKey () {
        int[] invKey = new int[subKey.length];
        int p = 0;
        int i = rounds * 6;
        invKey[i + 0] = mulInv(subKey[p++]);
        invKey[i + 1] = addInv(subKey[p++]);
        invKey[i + 2] = addInv(subKey[p++]);
        invKey[i + 3] = mulInv(subKey[p++]);
        for (int r = rounds - 1; r >= 0; r--) {
            ...
            invKey[i + 5] =        subKey[p++];
            invKey[i + 0] = mulInv(subKey[p++]);
            invKey[i + m] = addInv(subKey[p++]);
            invKey[i + n] = addInv(subKey[p++]);
            invKey[i + 3] = mulInv(subKey[p++]); }
        subKey =  invKey; }

    // Expands a 16-byte user key to the internal encryption sub-keys.
    private void makeSubKey (byte[] userKey) {
        this.subKey = new int[rounds * 6 + 4];
...
/*        final int minChar = 0x21;
        ...
        int[] a = new int[8];
        for (int p = 0; p < 100; p++) {
            int c = minChar + rnd.nextInt(maxChar - minChar + 1);
    ...
    public void setKey(String key) {
        ...
        for (int i = 0; i < buff.length; i++) {
            this.key[i] = Byte.parseByte(buff[i]);
        }
        makeSubKey(this.key);
    }
}